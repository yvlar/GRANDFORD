...
| **Version** | 0.12.1 |
| **Phase active** | v1.1 |
| **Sprint actif** | **Sprint 13 — Journal des changements (FR-13)** |
| **Dernier sprint complété** | Sprint 12 — Landing page SaaS (refonte ui-ux-pro-max) ✅ |
...
### Sprint 12 — Landing page SaaS (refonte ui-ux-pro-max) ✅
**Livré v0.12.1** : refonte visuelle complète de la landing page via skill `ui-ux-pro-max`. **Design system** : Flat Design, couleurs Primary `#2563EB` / CTA `#F97316`, typographie Plus Jakarta Sans. **Sections redessinées** : Navbar sticky épurée + **Hero gradient bleu** (badge, formes géométriques, CTA orange + ghost) + **Stats bar** (≤ 3 taps · Pitman · 0 % motifs partagés) + Problème + **Bento Grid** features (carte bleue featured 2-rangées pour Capture, card blanche Rappels, card orange Confidentialité) + **Personas** (Travailleur / Conjoint(e), cartes pleine-largeur colorées) + CTA final (fond bleu) + Footer. **Nouvelles strings i18n** dans `fr.landing` : `hero.badge`, `stats.*`, `pourQui.corpsTravailleur`, `pourQui.corpsConjointe`, `ctaFinal.sous`, `fonctionnalites.capture.tags`. Server Component pur, SVG inline, aucun nouveau package. R7 : 0 motif exposé. Gates mesurés : vitest **175** (inchangé), tsc 0, biome 0. Branche : `claude/sprint12-landing-page`.
