...
| **Version** | 0.9.0 |
| **Phase active** | v1.1 |
| **Sprint actif** | **Sprint 9 — Co-planification conjointe** |
| **Dernier sprint complété** | Sprint 8 — Mise en ligne + filets ✅ |
...
> Sprints 1-4 archivés : `docs/roadmap-archive.md`.
...
### Sprint 9 — Co-planification conjointe 🟡 ACTIF
**Objectif** : FR-8 (notes de la conjointe sur une date) + FR-9 (requêtes approuver/refuser — mécanisme de fraîcheur O-1). Prérequis Sprint 7 : déplacer la génération des rappels de la RPC `create_exception_with_motif` vers un **trigger sur `exceptions`** (sinon les écarts issus de requêtes n'auront pas de rappels). **Carte détaillée** : `prompt-mise-a-jour-roadmap.md`.

## Horizons post-MVP (non planifiés en sprints)

- **v1.1 restant** : journal des changements (FR-13), export iCal/PDF (FR-14) ; conformité Loi 25 opérationnalisée dès le 2ᵉ foyer.
