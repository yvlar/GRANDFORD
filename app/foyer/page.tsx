    ...
    <main className="min-h-dvh bg-slate-950 text-slate-50">
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6">
        ...
        {erreur ? (
          <p
            role="alert"
            className="mb-6 rounded-xl border border-red-800 bg-red-950 px-4 py-3 text-sm text-red-200"
          >
            {erreur}
          </p>
        ) : null}

        {/* ─── Grille desktop : gauche = membres/invitation · droite = réglages ── */}
        <div className="flex flex-col gap-4 lg:grid lg:grid-cols-[1fr_320px] lg:items-start lg:gap-6">
          {/* ── COLONNE GAUCHE ────────────────────────────────────── */}
          <div className="flex flex-col gap-4">
            {/* Membres */}
            <section className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900 px-5 py-5">
              ...
              {estProprietaire ? (
                <>
                  <form action={creerInvitation.bind(null, foyer.id)}>
                    <button
                      type="submit"
                      className="w-full cursor-pointer rounded-xl bg-blue-600 px-4 py-3 text-base font-semibold text-white transition-colors duration-200 hover:bg-blue-500"
                    >
                      {t.inviter}
                    </button>
                  </form>
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <ul className="flex flex-col gap-2">
                        {invitations.map((invitation) => (
                          <li
                            key={invitation.id}
                            className="flex flex-col gap-2 rounded-xl border border-slate-800 bg-slate-950 px-4 py-4"
                          >
                            <span className="text-xs font-semibold uppercase tracking-widest text-slate-500">
                              {t.copierLien}
                            </span>
                            <code className="break-all text-sm text-blue-300">{`${origin}/invitation/${invitation.code}`}</code>
                ...
                <form action={quitterFoyer.bind(null, monAdhesion.id)}>
                  <button
                    type="submit"
                    className="cursor-pointer rounded-xl border border-slate-700 px-4 py-3 font-semibold text-slate-300 transition-colors duration-200 hover:border-slate-600 hover:bg-slate-800"
                  >
                    {t.quitterFoyer}
                  </button>
                </form>
          ...
          {/* ── COLONNE DROITE : réglages personnels ──────────────── */}
          <div className="flex flex-col gap-4">
            {/* Travailleur (nom) */}
            <section className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900 px-5 py-5">
              <h2 className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500">
              ...
              {monAdhesion.role === "worker" ? (
                <form action={mettreAJourNom} className="flex flex-col gap-2">
                  <input
                    ...
                    maxLength={120}
                    className="rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-base text-slate-50 placeholder:text-slate-600 transition-colors duration-200 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                  />
                  <button
                    type="submit"
                    className="cursor-pointer self-start rounded-xl border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-200 transition-colors duration-200 hover:border-slate-600 hover:bg-slate-800"
                  >
                    {t.enregistrerNom}
                  </button>
                </form>
            ...
            <section className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900 px-5 py-5">
              ...
              {icalToken ? (
                <a
                  ...
                  className="inline-flex cursor-pointer items-center gap-2 self-start rounded-xl border border-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-200 transition-colors duration-200 hover:border-slate-600 hover:bg-slate-800"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="none"
