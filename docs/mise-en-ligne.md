...
- **Sentry** (scaffolding R7-strict **livré**, Option A — inerte sans DSN) :
  `@sentry/nextjs` câblé via `instrumentation.ts` + `sentry.{client,server,edge}.config.ts`
  + `withSentryConfig` (`next.config.mjs`) + `app/global-error.tsx`. `sendDefaultPii: false`
  et un `beforeSend` qui passe par `lib/monitoring/scrub.ts` (rempart R7 testé) : identité,
  `extra`, corps/cookies/en-têtes de requête et variables locales de pile retirés ;
  courriels et dates ISO caviardés dans messages, étiquettes, nom de transaction, threads.
  **Au go-live** : poser `NEXT_PUBLIC_SENTRY_DSN` (+ `SENTRY_DSN` côté serveur) sur Vercel
  → `enabled` devient vrai et l'envoi démarre ; envoyer un événement de test et vérifier
  dans Sentry qu'il ne porte **ni courriel, ni motif, ni date d'absence**. Source maps
  (optionnel) : `SENTRY_AUTH_TOKEN`/org/project pour activer l'upload (sinon muet).
  ⚠️ Turbopack : `sentry.client.config.ts` est déprécié au profit de `instrumentation-client.ts`
  (Next ≥ 15.3) — non bloquant sous Next 15.1.6 (build webpack) ; renommer à l'upgrade.
...
- Intégration **Sentry** : **Option A retenue** — scaffolding R7-strict (`beforeSend`
  filtrant + `sendDefaultPii: false`) livré et testé, SDK inerte tant qu'aucun DSN n'est
  posé. Reste au go-live : brancher le DSN réel et constater l'événement de test sans
  donnée sensible (cf. §3.5 et §4).
