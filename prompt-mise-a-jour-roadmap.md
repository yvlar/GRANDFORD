...
**Migration Cloud Sprint 25 — À APPLIQUER (priorité haute, désormais bloquante pour `/epicerie`)** : `20260628120000_sprint25_grocery_lists.sql` (tables `grocery_lists`/`grocery_items`, trigger, RLS, RPC `cocher_element_epicerie`, publication Realtime) **pas encore confirmée sur Cloud** (projet `eirlzscgxeybyeinpmsi`, via Supabase MCP après confirmation). Sans elle, la page `/epicerie` et la pastille d'accueil échouent en prod (la pastille dégrade à 0, mais la page lève). Puis **régénérer les types** (le générateur MCP retire `graphql_public` — ne pas écraser le fichier du dépôt ; `lib/database.types.ts` a été hand-edité fidèlement). **Déployer l'Edge `notify-grocery`** (+ confirmer `SUPABASE_SERVICE_ROLE`/`SUPABASE_FUNCTIONS_URL` côté Vercel).
...
**Sprint 27 — aucune migration propre** (frontend + requête de lecture), mais voir la migration Sprint 25 ci-dessus dont `/epicerie` dépend.
